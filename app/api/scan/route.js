...
export async function GET(request) {
  ...
  const requireAllYearsAboveMinAvg = searchParams.get('requireAllYearsAboveMinAvg') === 'true';
  ...
  for (const symbol of batch) {
    ...
    const allYearsAboveMinAvg = stats.yearsAvailable === years && qualifyingYears === years;
    if (stats.average >= minAvg && stats.positiveYears >= minPositive && (!requireAllYearsAboveMinAvg || allYearsAboveMinAvg)) {
      results.push({ symbol, name: stock.name || symbol, instrumentKey: stock.instrumentKey, qualifyingYears, allYearsAboveMinAvg, ...stats });
    }
  }

  results.sort((a, b) => b.average - a.average);
  return NextResponse.json({ ok:true, source: universeName === 'test10' ? (marketData.source || 'Stored monthly dataset') : 'Upstox historical monthly files', dataUpdatedAt: universeName === 'test10' ? marketData.updatedAt : null, historyYears: universeName === 'test10' ? marketData.historyYears : 10, universe: universeName, month, years, completedMonthOnly:true, minAvg, requireAllYearsAboveMinAvg, offset, limit, batchCount:batch.length, totalUniverse:allSymbols.length, scanned:batch.length, matched:results.length, results, errors });
}
