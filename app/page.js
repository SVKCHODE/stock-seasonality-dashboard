...
export default function Home(){
 ...
 return <main style={{maxWidth:1150,margin:'0 auto',padding:'30px 18px',fontFamily:'Arial, sans-serif'}}>
  <div style={{fontSize:12,fontWeight:800,letterSpacing:1.2}}>MARKET RESEARCH TOOL · V0.6</div>
  <h1 style={{fontSize:34,margin:'8px 0'}}>Seasonal Stock Scanner</h1>
  <p style={{color:'#667085',maxWidth:780}}>Screen NSE stocks for monthly seasonality using stored Upstox historical monthly candles. Scans read the local historical dataset, so normal scans do not call Upstox.</p>
  <section style={{background:'#fff',border:'1px solid #e4e7ec',borderRadius:14,padding:20,marginTop:24}}>
   ...
   <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:14}}>
    ...
    <label>Universe<select value={universe} onChange={e=>setUniverse(e.target.value)} style={input}><option value="nifty500">NSE Nifty 500</option><option value="test10">Initial 10-stock test universe</option></select></label>
  ...
  <section style={{marginTop:22}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><h2>{scanned?'Results':'Run your first scan'}</h2><span style={{color:'#667085',fontSize:13}}>{data?`${data.matched} matched / ${data.scanned} scanned`:'Stored-data scan'}</span></div>
   ...
   <p style={{fontSize:12,color:'#667085',marginTop:12}}>Method: previous month-end close → selected month-end close. The current incomplete month is excluded when it is the selected month. Historical data is refreshed periodically by GitHub Actions; normal scans do not call Upstox.</p>
  </section>
 </main>
}
