...
export default function Home(){
 ...
 return <main style={{maxWidth:1150,margin:'0 auto',padding:'30px 18px',fontFamily:'Arial, sans-serif'}}>
  <div style={{fontSize:12,fontWeight:800,letterSpacing:1.2}}>MARKET RESEARCH TOOL · V0.4</div>
  <h1 style={{fontSize:34,margin:'8px 0'}}>Seasonal Stock Scanner</h1>
  <p style={{color:'#667085',maxWidth:780}}>Validate our seasonality calculation against stocks with a shorter observable history before scaling to Nifty 500. Real Upstox monthly candles are used for every scan.</p>
  ...
  <section style={{marginTop:22}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><h2>{scanned?'Results':'Run your first scan'}</h2><span style={{color:'#667085',fontSize:13}}>{data?`${data.matched} matched / ${data.scanned} scanned`:'Real-data scan'}</span></div>
   ...
   <p style={{fontSize:12,color:'#667085',marginTop:12}}>Method: previous month-end close → selected month-end close. The current incomplete month is excluded when it is the selected month. The yearly breakdown is intentionally exposed so we can compare every observation against Moneycontrol before expanding the universe.</p>
  </section>
 </main>
}
